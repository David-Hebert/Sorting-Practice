...
const austinArea = () => {
  let arr = stringToNum(strNums)
  for (i = 0; i < arr.length; i++) {
    if (arr[i] + i === 512) {
      console.log(`index: ${i}, value: ${arr[i]} Equals: ${i + arr[i]}`);
      return arr[i] + i;
    }
  }
};
...
const weatherStates = (arr) => {
  let weatherArr = [];
  for(elem of arr) {
    if (weatherArr.indexOf(elem.weather_state_name) === -1) {
      weatherArr.push(elem.weather_state_name)
    }
  }
